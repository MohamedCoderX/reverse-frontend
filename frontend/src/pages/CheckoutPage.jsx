...
const CheckoutPage = () => {
    ...
    const fetchUserProfile = async () => {
      ...
      try {
        ...
        if (data.shippingAddress?.address) {
          ...
          // Sanitize phone numbers - trim spaces and keep as-is
  ...
  const handlePayment = async (e) => {
    ...
    if (cleanPhone.length < 10) { toast.error("Please enter a valid phone number (at least 10 digits)"); return; }
    if (formData.altPhone) {
      const cleanAlt = formData.altPhone.replace(/\D/g, '');
      if (cleanAlt.length < 10) { toast.error("Please enter a valid alternate phone number (at least 10 digits)"); return; }
    }
  ...
  const proceedToPayment = async () => {
    ...
    try {
      ...
      const options = {
        ...
        prefill: { name: formData.fullName, email: user?.email || '', contact: formData.phone.replace(/\D/g, '') },
        theme: { color: '#064e3b' },
      };
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          <input type="tel" required name="phone" value={formData.phone} onChange={(e) => {
                            ...
                            className={`w-full px-6 py-4 bg-[#fdfbf7] border rounded-2xl focus:outline-none font-medium text-[#064e3b] transition-all ${formData.phone && formData.phone.replace(/\D/g, '').length < 10 && formData.phone.length > 0 ? 'border-red-500 focus:border-red-500' : 'border-[#064e3b]/10 focus:border-[#c5a059]'}`} placeholder="Enter mobile number" maxLength={17} />
                        </div>
                        {formData.phone && formData.phone.length > 0 && formData.phone.replace(/\D/g, '').length < 10 && (
                          <p className="text-red-500 text-xs font-medium ml-1">Please enter a valid phone number (at least 10 digits)</p>
                        )}
                      </div>
                      <div className="space-y-2">
                        ...
                        <input type="tel" name="altPhone" value={formData.altPhone || ''} onChange={(e) => {
                          ...
                          className={`w-full px-6 py-4 bg-[#fdfbf7] border rounded-2xl focus:outline-none font-medium text-[#064e3b] transition-all ${formData.altPhone && formData.altPhone.replace(/\D/g, '').length < 10 && formData.altPhone.length > 0 ? 'border-red-500 focus:border-red-500' : 'border-[#064e3b]/10 focus:border-[#c5a059]'}`} placeholder="Enter mobile number" maxLength={17} />
                        {formData.altPhone && formData.altPhone.length > 0 && formData.altPhone.replace(/\D/g, '').length < 10 && (
                          <p className="text-red-500 text-xs font-medium ml-1">Please enter a valid phone number (at least 10 digits)</p>
                        )}
                      </div>
                    </div>
              ...
              <div className="bg-white rounded-[2.5rem] p-8 md:p-10 shadow-2xl border border-[#064e3b]/5 relative overflow-hidden">
                ...
                <div className="mt-10 pt-10 border-t border-[#064e3b]/5 text-center">
                  ...
                  <div className="flex items-center justify-center gap-4 grayscale opacity-40 hover:opacity-100 hover:grayscale-0 transition-all cursor-default">
                    <img src={new URL('../assets/GPAY.webp', import.meta.url).href} alt="GPay" className="h-5" />
                    <img src={new URL('../assets/PAYTYM.webp', import.meta.url).href} alt="Paytm" className="h-5" />
                    <img src={new URL('../assets/PHONEPE.webp', import.meta.url).href} alt="PhonePe" className="h-5" />
                  </div>
                </div>
              </div>
            </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-4 sm:mb-6">
                    ...
                    <div className="space-y-3 sm:space-y-4">
                      <div className="p-4 sm:p-5 bg-[#fdfbf7] rounded-xl sm:rounded-2xl">
                        ...
                        <p className="text-[#064e3b]/60 text-sm">{displayPhone(formData.phone)}</p>
                        {formData.altPhone && <p className="text-[#064e3b]/40 text-[10px] sm:text-xs mt-1">Alt: {displayPhone(formData.altPhone)}</p>}
                      </div>
